...
result_dir = root / "iupred3_expanded_raw"
...
for record in SeqIO.parse(root / "expanded_bacteriocins.fasta", "fasta"):
    ...
    if output.exists():
        ...
        payload = {"csrfmiddlewaretoken": token, "accession": "", "inp_seq": str(record.seq), "context": "long", "smoothing": "savgol"}
    ...
    rows.append({
        ...
        "nterm40_idr_fraction": sum(score >= 0.5 for score in scores[:40]) / min(40, len(scores)),
        "idr_segments": "; ".join(f"{start}-{end}" for start, end in found)
    })

pd.DataFrame(rows).to_csv(root / "expanded_iupred3_summary.csv", index=False)
